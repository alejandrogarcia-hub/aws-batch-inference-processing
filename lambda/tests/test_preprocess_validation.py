...
if "datasets" not in sys.modules:
    ...
    datasets_stub.load_dataset = (
        lambda *args, **kwargs: None
    )  # pragma: no cover - not exercised in these tests
...
class LambdaHandlerMaxRecordsTests(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.write_patch = mock.patch(
            "preprocess.write_jsonl_to_s3", return_value="s3://bucket/key"
        )
    ...
    def test_without_limit_processes_all_chunks(self) -> None:
        ...
        def _yield_chunks(*args, **kwargs):
            for chunk_index in range(
                60
            ):  # 60 chunks of 1000 records => 60k records raw
                yield (
                    chunk_index,
                    _StubDataFrame(
                        {"input_text": f"text-{chunk_index}-{i}"} for i in range(1000)
                    ),
                )
    ...
    def test_respects_max_records_total(self) -> None:
        ...
        self.assertEqual(
            len(result["jobs"]), 3
        )  # two full chunks + one truncated chunk
        load_mock.assert_called_once()
