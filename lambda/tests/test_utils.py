...
class LoadFilesInChunksTests(unittest.TestCase):
    ...
    def test_loads_csv_chunks(self) -> None:
        ...
        results = list(load_files_in_chunks('s3://bucket/input.csv', 'csv', chunk_size=100))
    ...
    def test_loads_parquet_chunks(self) -> None:
        ...
        results = list(load_files_in_chunks('s3://bucket/input.parquet', 'parquet', chunk_size=256))

        self.assertEqual(results, list(enumerate(mock_read_parquet.return_value)))
        mock_read_parquet.assert_called_once_with('s3://bucket/input.parquet', chunked=256)
