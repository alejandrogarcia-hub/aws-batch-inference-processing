...
import boto3
import utils
from custom_types import JobStatus, TaskItem
...
def lambda_handler(event, context) -> dict[str, Any]:
    ...
    task_item_ddb = TaskItem.model_validate(
        task_table.get_item(
            Key={
                "job_arn": job_arn,
            }
    ...
    logger.info(f"Current job status: {job_status.value}")
    ...
    if job_status == JobStatus.COMPLETED:
        logger.info(
            "Job completed successfully. Sending task success to step function."
        )
        sfn_client.send_task_success(
            taskToken=task_token,
            output=json.dumps(task_item_dict),
        )
    elif job_status in BATCH_INFERENCE_ERROR_STATES:
        logger.info(
            f"Task failed with status {job_status.value}. Sending task failure to step function."
        )
        sfn_client.send_task_failure(
            taskToken=task_token,
            error=job_status.value,
        )
    else:
        logger.info(
            f"Job in-progress with status {job_status}. Sending task heartbeat to step function."
        )
