...
MAX_TOTAL_RECORDS: Optional[int] = None if _max_total_records_env is None or _max_total_records_env.strip() == '' else int(_max_total_records_env)
...
def resolve_prompt_configuration(model_type: str, event: JobInput) -> Tuple[str | None, Set[str]]:
    ...
    if not prompt_id:
        raise ValueError('Text models require a prompt_id that matches an entry in prompt_templates.prompt_id_to_template.')

    try:
        prompt_template = pt.prompt_id_to_template[prompt_id]
    except KeyError as exc:
        available = ', '.join(sorted(pt.prompt_id_to_template)) or '<none>'
        raise ValueError(f'Unknown prompt_id "{prompt_id}". Available prompt_ids: {available}.') from exc

    formatter = Formatter()
    required_fields = {field for _, field, _, _ in formatter.parse(prompt_template) if field}
...
def lambda_handler(event: JobInput, context) -> JobConfigList:
    ...
    prompt_template, required_prompt_fields = resolve_prompt_configuration(processor.model_type, event)

    max_num_jobs = event.get('max_num_jobs')
    max_records_per_job = event.get('max_records_per_job', MAX_RECORDS_PER_JOB)
    max_records_total_raw = event.get('max_records_total', MAX_TOTAL_RECORDS)
    ...
    for idx, input_df in utils.load_files_in_chunks(s3_uri, file_type, chunk_size=max_records_per_job):
        ...
        if max_records_total is not None and processed_records + len(input_df) > max_records_total:
            allowed = max_records_total - processed_records
            logger.info(f'Truncating chunk to {allowed} records to respect max_records_total {max_records_total}.')
        ...
        validate_chunk_columns(
            ...
            job_name_prefix=event['job_name_prefix'],
            prompt_id=event.get('prompt_id'),
        )
        ...
        if processor.model_type == 'embedding':
            ...
            records = [
                processor.process_input(
                    input_text=prompt_template.format(**r),
        ...
        if max_records_total is not None and processed_records >= max_records_total:
            logger.info('Reached max_records_total limit after current chunk. Stopping.')
            break
